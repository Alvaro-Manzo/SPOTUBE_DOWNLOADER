...
def load_usage():
    """Carga el uso del usuario desde el archivo."""
    if os.path.exists(USAGE_FILE):
        with open(USAGE_FILE, "r") as f:
            return json.load(f)
    return {"downloads": [], "pro_activated": False}


def save_usage(usage_data):
    """Guarda el uso del usuario."""
...
def check_free_limit():
    """
    Verifica si el usuario ha excedido el límite gratuito mensual.
    Returns: (can_download: bool, remaining: int, tier: str)
    """
    usage = load_usage()

    # Si tiene PRO, sin límites
    if usage.get("pro_activated", False):
        return True, "∞", "PRO"
...
def record_download():
    """Registra una descarga en el sistema."""
    usage = load_usage()
    now = datetime.now()
